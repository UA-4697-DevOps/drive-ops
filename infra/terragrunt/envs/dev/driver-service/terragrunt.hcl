...
inputs = {
  # Context
  name         = "Training-${local.common_vars.project_name}-${local.env_vars.env}-driver-service"
  ...
  # ECR
  ecr_repository_url = dependency.shared_infra.outputs.ecr_repository_urls.driver_service

  # --- IAM Permissions ---

  # 1. Secret Access: RDS credentials from Secrets Manager
  iam_secret_arns = [
    dependency.shared_infra.outputs.rds_secret_arn
  ]

  # 2. SQS Access: all three queues
  iam_sqs_arns = [
