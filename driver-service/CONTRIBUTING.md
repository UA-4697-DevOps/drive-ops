# Contributing to Drive Service

This document outlines the workflow for contributing features and bug fixes specifically to the **Drive Service**. Please follow the guidelines below to ensure a clean and manageable history.

## Branching Strategy

We use a team-specific branching model. All work targeting this service should ultimately flow through the `drive-service` branch before reaching production (`main`).

### The Flow
`feature/<name>` —(Squash Merge)→ `drive-service` —(Merge Commit)→ `main`

### 1. Create a Feature Branch
For any new feature or fix, create a dedicated branch. **Do not** branch off `main` directly.
* **Base Branch:** `drive-service`
* **Naming Convention:** `feature/<short-description>` or `fix/<short-description>`

```bash
# Example
git checkout drive-service
git pull origin drive-service
...
### Step 1: Merge to Team Branch (`drive-service`)

When your task is complete:

1. Open a **Pull Request (PR)** targeting the `drive-service` branch.
...
* This ensures the `drive-service` history remains clean, with one commit per feature/task.
...
* **Full Update:** Use the `drive-service` branch directly.
* **Specific Release:** Create a new branch pointing to a specific commit on `drive-service` (e.g., if you only want to release changes up to a certain point).
