...
def driver_menu():
    keyboard = [[KeyboardButton(BTN_MY_ORDERS)], [KeyboardButton(BTN_CHANGE_ROLE)]]
    return ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
...
async def change_role(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_orders.pop(chat_id, None)
    await update.message.reply_text(
        "\U0001f504 Оберіть нову роль:", reply_markup=role_selection_menu()
    )
...
def main():
    ...
    application.add_handler(
        MessageHandler(filters.Regex(f"^{re.escape(BTN_CHANGE_ROLE)}$"), change_role)
    )
