...
def register_handlers(application, user_orders, user_roles, buttons, keyboards, helpers):
            ...
            [
                [
                    InlineKeyboardButton(
                        "\U0001f695 Замовити таксі зараз", callback_data="quick_order_taxi"
                    )
                ]
            ]
        )
    ...
    async def cancel_order_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if chat_id in user_orders:
            ...
            await safe_send(
                chat_id,
                "\u274c Ваше незавершене замовлення скасовано.",
                context,
            ...
            await safe_send(
                chat_id,
                "У вас немає активного незавершеного замовлення.",
                context,
                reply_markup=get_user_menu(chat_id),
            )
    ...
    async def start_order_flow(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if chat_id in user_orders and user_orders[chat_id].get("_in_progress"):
            await update.message.reply_text(
                "У вас вже є незавершене замовлення. Скасуйте командою /cancel_order або завершіть поточне."
            )
            return ConversationHandler.END
    ...
    async def process_pickup_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if chat_id not in user_orders or not user_orders.get(chat_id):
            await safe_send(
                chat_id,
                "❌ Замовлення скасовано або закінчилось. Спробуйте знову.",
                context,
                reply_markup=get_user_menu(chat_id),
            )
            return ConversationHandler.END
    ...
    async def process_comment_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
        chat_id = update.effective_chat.id
        logger.info(
            "process_comment_step called for chat_id=%s, text=%s", chat_id, update.message.text
        )
        ...
        logger.info(
            "Order data: pickup=%s, dropoff=%s, comment=%s",
            order.get("pickup"),
            order.get("dropoff"),
            order.get("comment"),
        )
    ...
    async def handle_quick_order_taxi(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if chat_id not in user_roles:
            await context.bot.send_message(
                chat_id, "Спочатку оберіть вашу роль за допомогою команди /start"
            )
            return ConversationHandler.END
    ...
    async def handle_order_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            if query.data == "order_confirm":
                ...
                await query.edit_message_text(text="❌ Замовлення скасовано.")

            chat_id = query.message.chat.id
            await context.bot.send_message(
                chat_id, "Головне меню:", reply_markup=get_user_menu(chat_id)
            )
        finally:
    ...
    application.add_handler(
        MessageHandler(filters.Regex(f"^{re.escape(BTN_PASSENGER)}$"), select_passenger_role)
    )
