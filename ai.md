...
* Step 6-7: Trip Service triggers matching by sending a message to the `trip.created` SQS queue. Driver Service polls and receives messages from this queue.
...
* Step 15-16: Driver Service sends a message to the `driver.assigned` SQS queue. Trip Service polls and receives messages from this queue to sync the state.
